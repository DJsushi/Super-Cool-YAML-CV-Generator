...
fn main() -> anyhow::Result<()> {
    ...
    // let yaml_content = fs::read_to_string(&cli.cv).unwrap_or_else(|e| {
    //     println!("Failed to read CV file: {}", e);
    //     exit(1);
    // });
    //
    // let json_value: serde_json::Value = serde_yaml::from_str(&yaml_content).unwrap_or_else(|e| {
    //     println!("Failed to parse YAML: {}", e);
    //     exit(1);
    // });
    //
    // let schema: serde_json::Value = serde_json::from_str(include_str!("cv-schema.json"))
    //     .unwrap_or_else(|e| {
    //         println!("Failed to parse schema: {}", e);
    //         exit(1);
    //     });
    //
    // jsonschema::validate(&schema, &json_value).unwrap_or_else(|e| {
